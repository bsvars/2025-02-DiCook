...
% Rubio-Ramirez (2019).
% This file includes a measure of credit spreads in the VAR and imposes
% additional identifying restrictions related to this variable.
...
DATA = table2array(DATA(date>=datetime(1993,3,1) & date<datetime(2020,1,1),2:end));
...
data = [DATA(:,1), 100*log(DATA(:,2:4)) DATA(:,9)];

% Extract proxy variable.
proxy = DATA(:,8);

% Set variable names.
varnames = {'Cash Rate','Real GDP','CPI','TWI','Spread'};

%% Options.
opt.p = 4; % No. of lags in VAR
opt.const = 1; % const = 1 if constant in VAR, = 0 otherwise
opt.ivar = 1:5;  % Indices of variables of interest
...
restr.signRestr = ...
      [1 2 0 -1 2; % Coefficient on real GDP is nonpositive
       ...
       1 5 0 1 2; % Coefficient on spread is nonnegative
       ...
       5 1 0 1 1; % Impact response of spread is nonnegative
       5 1 1 1 1; % As above after one quarter
       5 1 2 1 1; % As above after two quarters
       5 1 3 1 1]; % As above after three quarters
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT1,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestr_Cred'));
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT2,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestrRel_Cred'));
...
for ii = 1:length(hors)
        ...
        lowerProbs(ii,:,1) = mean(relub(:,hors(ii),2) <= thresh);
        lowerProbs(ii,:,2) = mean(relub(:,hors(ii),3) <= thresh);
        
end

TT4 = table(hors-1,100*lowerProbs(:,:,1),100*lowerProbs(:,:,2));
TT4.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT4,'TableData.xlsx','Sheet','LowerProbs_Cred');

upperProbs = zeros(length(hors),length(thresh),2);

for ii = 1:length(hors)
        
        upperProbs(ii,:,1) = mean(rellb(:,hors(ii),2) <= thresh);
        upperProbs(ii,:,2) = mean(rellb(:,hors(ii),3) <= thresh);
        
end

TT5 = table(hors-1,100*upperProbs(:,:,1),100*upperProbs(:,:,2));
TT5.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT5,'TableData.xlsx','Sheet','UpperProbs_Cred');

cd(oldFolder);

save('proxyRestr_Cred_results.mat');