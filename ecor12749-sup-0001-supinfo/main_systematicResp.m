...
% sign restrictions on the systematic component of monetary policy,
% as in Arias, Caldara and Rubio-Ramirez (2019).
% Restrictions (3) and (4) in paper.
...
restr.signRestr = ...
      [1 1 0 1 1; % Impact response of cash rate is nonnegative
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_systematicResp'));
    
end

cd(oldFolder);

save('systematicResp_results.mat');
...
for ii = 1:length(opt.ivar)
    ...
    TT.Properties.VariableNames = {'Horizon','Mean','HPDLB','HPDUB',...
        'MeanLB','MeanUB','RCLB','RCUB'};
    writetable(TT,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_systematicRespTWI'));
    
end

cd(oldFolder);

save('systematicRespTWI_results.mat'); 
