...
% sign restrictions on impulse responses, as in Uhlig (2005).
% Restrictions (1) and (2) in paper.
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_signRestr'));
    
end

cd(oldFolder);

save('signRestr_results.mat');

%% Impose additional restrictions on response of exchange rate.
% Each row of signRestr contains a vector (i,j,h,s,t) representing a
% 'traditional' sign restriction, where t is the type of restriction:
% t = 1: the impulse response of the ith variable to the jth shock at the 
% hth horizon is nonnegative (s = 1) or nonpositive (s = -1).
...
restr.signRestr = ...
      [restr.signRestr;
       4 1 0 1 1; % Response of TWI to monetary policy shock on impact is nonnegative
...
for ii = 1:length(opt.ivar)
    ...
    TT.Properties.VariableNames = {'Horizon','Mean','HPDLB','HPDUB',...
        'MeanLB','MeanUB','RCLB','RCUB'};
    writetable(TT,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_signRestrTWI'));
    
end

cd(oldFolder);

save('signRestrTWI_results.mat');
