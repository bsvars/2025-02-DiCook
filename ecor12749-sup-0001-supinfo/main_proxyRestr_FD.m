...
% Rubio-Ramirez (2019).
% This file estimates the reduced-form model in first differences of some 
% variables.
...
data = [DATA(2:end,1), ... 
    100*(log(DATA(2:end,2:3))-log(DATA(1:end-1,2:3))), ...  
    100*log(DATA(2:end,4))];
...
opt.cumIR = [2:3]; % Indices of variables for cumulative impulse responses 
...
exog = [100*(log(DATA(2:end,5:6))-log(DATA(1:end-1,5:6))), DATA(2:end,7)];
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT1,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestr_FD'));
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT2,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestrRel_FD'));
...
thresh = -(0.1:0.1:0.5); % Thresholds of interest

lowerProbs = zeros(length(hors),length(thresh),2);

for ii = 1:length(hors)
        
        lowerProbs(ii,:,1) = mean(etaub(:,hors(ii),2) <= thresh);
        lowerProbs(ii,:,2) = mean(etaub(:,hors(ii),3) <= thresh);
        
end

TT4 = table(hors-1,100*lowerProbs(:,:,1),100*lowerProbs(:,:,2));
TT4.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT4,'TableData.xlsx','Sheet','LowerProbs_FD');

upperProbs = zeros(length(hors),length(thresh),2);

for ii = 1:length(hors)
        
        upperProbs(ii,:,1) = mean(etalb(:,hors(ii),2) <= thresh);
        upperProbs(ii,:,2) = mean(etalb(:,hors(ii),3) <= thresh);
        
end

TT5 = table(hors-1,100*upperProbs(:,:,1),100*upperProbs(:,:,2));
TT5.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT5,'TableData.xlsx','Sheet','UpperProbs_FD');

cd(oldFolder);

save('proxyRestr_FD_results.mat');