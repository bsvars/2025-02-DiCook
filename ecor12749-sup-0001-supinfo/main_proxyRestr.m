...
opt.dispIter = 10; % Print number of draws remaining every dispIter draws
...
% (s = -1). 
% Assumes sign restriction on impact response of FFR to monetary policy
% shock is ordered first.
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT1,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestr'));
...
for ii = 1:length(opt.ivar)
    ...
    writetable(TT2,'FigureData.xlsx','Sheet',strcat(varnames{ii},'_proxyRestrRel'));
...
thresh = -(0.25:0.25:2); % Thresholds of interest

lowerProbs = zeros(length(hors),length(thresh)+1,2);

for ii = 1:length(hors)
        
        lowerProbs(ii,:,1) = [mean(etaub(:,hors(ii),2) <= 0), ... 
        mean(relub(:,hors(ii),2) <= thresh)];
        lowerProbs(ii,:,2) = [mean(etaub(:,hors(ii),3) <= 0), ...
            mean(relub(:,hors(ii),3) <= thresh)];
        
end

TT4 = table(hors-1,100*lowerProbs(:,:,1),100*lowerProbs(:,:,2));
TT4.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT4,'TableData.xlsx','Sheet','LowerProbs');

upperProbs = zeros(length(hors),length(thresh)+1,2);

for ii = 1:length(hors)
        
        upperProbs(ii,:,1) = [mean(etalb(:,hors(ii),2) <= 0), ...
            mean(rellb(:,hors(ii),2) <= thresh)];
        upperProbs(ii,:,2) = [mean(etalb(:,hors(ii),3) <= 0), ...
            mean(rellb(:,hors(ii),3) <= thresh)];
        
end

TT5 = table(hors-1,100*upperProbs(:,:,1),100*upperProbs(:,:,2));
TT5.Properties.VariableNames = {'Horizon','Output','Prices'};
writetable(TT5,'TableData.xlsx','Sheet','UpperProbs');

cd(oldFolder);

save('proxyRestr_results.mat');